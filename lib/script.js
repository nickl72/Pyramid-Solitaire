...
function selectCard(event) {
    ...
    if (!cardSelected) { // first selection action
        ...
        return;
    } else if (event.target.classList.contains('selected')) { // toggle card class
    ...
    if (previouslySelected.attributes.card.value + event.target.attributes.card.value !== 13) {
        return;
    }
